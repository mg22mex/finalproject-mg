#!/usr/bin/env python3
"""
Configure Facebook Account: Auto Account 1
...
def configure_auto_account_1_account():
    """Configure Auto Account 1 Facebook account"""
    ...
    try:
        ...
        existing_account = db.query(FacebookAccount).filter(
            FacebookAccount.name == "Auto Account 1"
        ).first()
        ...
        print(f"✅ Auto Account 1 account configured successfully!")
        ...
        print(f"❌ Error configuring Auto Account 1: {str(e)}")
        db.rollback()
        return None
    finally:
        db.close()
...
if __name__ == "__main__":
    account_id = configure_auto_account_1_account()
    if account_id:
        print(f"\n🎉 Auto Account 1 Facebook account configured with ID: {account_id}")
    else:
        print(f"\n❌ Failed to configure Auto Account 1 Facebook account")
